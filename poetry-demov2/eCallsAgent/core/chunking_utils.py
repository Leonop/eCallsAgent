...
def process_chunk_worker(chunk_idx: int, chunk_docs: List[str], chunk_embeddings: np.ndarray,
    ...
    try:
        ...
        # Adjust UMAP parameters based on chunk size
        chunk_size = len(chunk_docs)
        if chunk_size <= n_components:
            # If chunk size is smaller than n_components, reduce n_components
            n_components = max(2, chunk_size - 1)  # Ensure at least 2 components
            n_neighbors = max(2, min(n_neighbors, chunk_size - 1))  # Ensure n_neighbors is valid
            logger.info(f"Adjusted UMAP parameters for small chunk: n_components={n_components}, n_neighbors={n_neighbors}")
