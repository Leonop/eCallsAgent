...
class TopicVis:
    ...
    def save_hierarchy(self, topic_model: BERTopic, custom_labels: dict, base_config: dict) -> None:
        ...
        try:
            # Ensure custom_labels is a dictionary and contains all topic indices
            if not isinstance(custom_labels, dict):
                custom_labels = {}
            
            # Get all valid topic indices from the model
            valid_topics = set(topic_model.get_topic_info()["Topic"].tolist())
            
            # Create a complete mapping of topic indices to labels
            complete_labels = {}
            for topic in valid_topics:
                if topic in custom_labels:
                    complete_labels[topic] = custom_labels[topic]
                else:
                    complete_labels[topic] = f"Topic {topic}"
            
            # Generate the hierarchy visualization
            fig = topic_model.visualize_hierarchy(
                custom_labels=complete_labels,
            ...
            self.logger.error(f"Error generating hierarchy visualization: {e}")
            self.logger.error(traceback.format_exc())
            # Create a fallback visualization
            fig = go.Figure()
            fig.add_annotation(
                text=f"Hierarchy visualization unavailable:<br>{str(e)}",
                xref="paper", yref="paper", x=0.5, y=0.5,
                showarrow=False, font=dict(size=14, color="red")
            )
            fig.update_layout(title="Topic Hierarchy (Unavailable)", width=1600, height=900)
            self.save_figure(fig, 'hierarchy_fallback', scale=1)
