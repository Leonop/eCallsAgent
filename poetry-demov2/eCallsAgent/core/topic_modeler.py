...
class TopicModeler:
    ...
    def _transform_documents_gpus(self, docs: list, embeddings: np.ndarray, chunk_size: int = 5000) -> tuple:
        """
        ...
        try:
            ...
            for gpu_id in range(gpus_per_node):
                ...
                gpu_docs = docs[start_idx:end_idx]
                gpu_embeddings = embeddings[start_idx:end_idx]
                
                # Move data to GPU
                gpu_embeddings = torch.tensor(gpu_embeddings, device=f'cuda:{gpu_id}')
                
                # Transform on this GPU
                gpu_topics, gpu_probs = self.topic_model.transform(gpu_docs, gpu_embeddings)
                
                # Move results back to CPU
                if isinstance(gpu_topics, torch.Tensor):
                    gpu_topics = gpu_topics.cpu().numpy()
                if isinstance(gpu_probs, torch.Tensor):
                    gpu_probs = gpu_probs.cpu().numpy()
                ...
                # Log progress
                self.logger.info(f"Completed processing on GPU {gpu_id}, processed {len(gpu_topics)} documents")
            
            # Ensure lengths match
            if len(all_topics) != len(docs) or len(all_probs) != len(docs):
                self.logger.warning(f"Length mismatch detected: docs={len(docs)}, topics={len(all_topics)}, probs={len(all_probs)}")
                # Pad or truncate to match document length
    ...
    def _train_final_model(self, all_rep_docs: list, all_rep_embeddings: list) -> BERTopic:
        ...
        try:
            ...
            # Instead of directly calling _cpu_topic_model, check if GPU acceleration is available
            
            self.logger.info("Attempting to create GPU-accelerated models for topic modeling")
            
            # Use local GPU models if CUML is available, otherwise use CPU models
            if CUML_AVAILABLE:
                try:
                    # Create UMAP model with cuML
                    umap_model = cumlUMAP(
                    ...
                    # Create HDBSCAN model with cuML
                    ...
                    self.logger.info("Successfully created GPU-accelerated models")
                except Exception as e:
                    self.logger.error(f"Error creating GPU models: {e}, falling back to CPU models")
                ...
                self.logger.warning("cuML not available, using CPU models instead")
                # Use CPU models
                umap_model = UMAP(
                    n_neighbors=n_neighbors,
                    n_components=n_components,
            ...
            self.logger.info(f"Created topic model with UMAP and HDBSCAN")
            ...
            self.logger.info("Merging similar topics...")
            topic_model = self.merge_similar_topics(topic_model, similarity_threshold=0.7)
    ...
    def _map_documents(self, docs: list, embeddings: np.ndarray) -> tuple:
        ...
        try:
            ...
            if len(docs) != len(embeddings):
                self.logger.error(f"Input dimension mismatch: docs={len(docs)}, embeddings={len(embeddings)}")
                raise ValueError("Document and embedding dimensions must match")
            ...
            if self.device == 'cuda' and torch.cuda.is_available():
                ...
                # Ensure lengths match
                if len(topics) != len(docs) or len(probs) != len(docs):
                    self.logger.warning(f"Length mismatch detected: docs={len(docs)}, topics={len(topics)}, probs={len(probs)}")
                    # Pad or truncate to match document length
                    if len(topics) < len(docs):
                        topics.extend([-1] * (len(docs) - len(topics)))
                        probs.extend([[0.0]] * (len(docs) - len(probs)))
                    else:
                        topics = topics[:len(docs)]
    ...
    def _create_topic_model(self, umap_model=None, hdbscan_model=None, _cpu = False):
        ...
        try:
            ...
            vectorizer_model = CountVectorizer(
                ...
                min_df=gl.MIN_DF[0],
                max_df=gl.MAX_DF[0],
                ngram_range=(2, 3),  # Add bigrams and tri-grams for better topic modeling
                )
    ...
    def _process_chunks_CPU_parallel(self, docs: list, embeddings: np.ndarray, _cpu = True) -> dict:
        """Process documents in parallel chunks using CPU."""
        try:
            # Validate input dimensions
            if len(docs) != len(embeddings):
                self.logger.error(f"Length mismatch between docs ({len(docs)}) and embeddings ({len(embeddings)})")
                # Truncate to the shorter length to ensure consistency
                min_length = min(len(docs), len(embeddings))
                docs = docs[:min_length]
                embeddings = embeddings[:min_length]
                self.logger.warning(f"Truncated to {min_length} documents to ensure consistency")
    ...
    def merge_similar_topics(self, topic_model, similarity_threshold=0.7):
        """
        Merge similar topics based on cosine similarity.
        
        Args:
            topic_model: The trained BERTopic model
            similarity_threshold: Threshold for merging topics (0-1)
            
        Returns:
            Updated topic model with merged topics
        """
        try:
            ...
            for i in range(len(similarity_matrix)):
                ...
                for j in range(i + 1, len(similarity_matrix)):
                    if j in topic_mapping:
                        continue
                        
                    if similarity_matrix[i][j] > similarity_threshold:
                        # Merge topics by mapping j to i
                        topic_mapping[j] = i
            ...
            for topic in topic_model.topics_:
                if topic in topic_mapping:
                    new_topics.append(topic_mapping[topic])
                else:
                    new_topics.append(topic)
                    
            topic_model.topics_ = np.array(new_topics)
            
            # Calculate new topic sizes from topics_ array
            unique_topics, counts = np.unique(topic_model.topics_, return_counts=True)
            topic_model._topic_sizes = dict(zip(unique_topics, counts))
            ...
            if hasattr(topic_model, '_topic_vectors'):
                ...
                for topic, vector in topic_model._topic_vectors.items():
                    if topic in topic_mapping:
                        new_topic = topic_mapping[topic]
                        if new_topic in new_vectors:
                            # Average the vectors of merged topics
                            new_vectors[new_topic] = (new_vectors[new_topic] + vector) / 2
                        else:
                            new_vectors[new_topic] = vector
                    else:
                        new_vectors[topic] = vector
                topic_model._topic_vectors = new_vectors
            
            self.logger.info(f"Merged {len(topic_mapping)} similar topics")
            return topic_model
            
        except Exception as e:
            self.logger.error(f"Error merging similar topics: {e}")
            return topic_model
