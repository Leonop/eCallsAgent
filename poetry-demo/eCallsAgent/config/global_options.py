...
YEAR_END = 2015 # train the model on data from start year to this year
...
final_parameters = {
    "n_neighbors": 30,                    # max(12, min(10, 1_000_000 // 8000)) = max(12, 10) = 12
    "n_components": 100,                  # adjusted for large embedding + not A100 (see below)
    "min_dist": 0.05,                     # assuming average doc length < 200
    "min_cluster_size": 30,              # Reduces over-fragmentation; encourages merging noisy small clusters.
    "min_samples": 15,                    # relaxed for more clusters
    "cluster_selection_epsilon": 0.01,   # increased slightly for broader clusters
}
...
MAX_DOCS_PER_TOPIC = 250_000 # Maximum number of documents per topic to use for distillation

# Vectorizer parameters for c-TF-IDF computation
MAX_DF = [0.95]         # Retains filtering for terms appearing in over 95% of documents
MIN_DF = [0.05]        # Terms must appear in at least 5% of documents (optionally test [0.01, 0.05])
