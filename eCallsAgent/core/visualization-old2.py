...
class VisualizationManager:
    def __init__(self, topic_model: BERTopic, custom_labels: Optional[Dict[int, str]] = None):
    ...
    def _get_base_config(self, width=1600, height=900):
    ...
    def _save_figure(self, fig, filename: str, n_topics: int = None, scale: int = 1) -> None:
        ...
        try:
            ...
            formatted_name = self._get_formatted_filename(filename, n_topics)
    ...
    def _get_formatted_filename(self, filename: str, n_topics: int = None) -> str:
        """Generate a consistent filename with parameters.
    ...
    def save_barchart(self, top_n_topics: int) -> None:
        ...
        fig.update_layout(
            title=dict(
                text=f"Topic Frequency<br><sup>{len(topics)} topics, {len(freq_df)} total topics</sup>",
                font=dict(size=20, family="Arial, sans-serif"),
                x=0.5,
                xanchor='center'
            )
        )
        self._save_figure(fig, 'barchart', n_topics=len(self.topic_model.get_topic_freq()), scale=1)

    def save_hierarchy(self, orientation="left", topics=None, top_n_topics=None, width=1000, height=1000):
        ...
        if not hasattr(self.topic_model, 'hierarchical_topics_') or \
           (hasattr(self.topic_model, 'hierarchical_topics_') and 
            ( (isinstance(self.topic_model.hierarchical_topics_, pd.DataFrame) and self.topic_model.hierarchical_topics_.empty) or
              (isinstance(self.topic_model.hierarchical_topics_, dict) and len(self.topic_model.hierarchical_topics_) == 0) )):
            ...
            try:
                ...
                if hasattr(self.topic_model, 'get_representative_docs'):
                    docs_per_topic = self.topic_model.get_representative_docs()
                elif hasattr(self.topic_model, 'topic_representations_'):
                    docs_per_topic = pd.DataFrame({
                        'Topic': list(self.topic_model.topic_representations_.keys()),
                        'Doc_IDs': [docs for docs in self.topic_model.topic_representations_.values()]
                    })
                else:
                ...
                if docs_per_topic.empty or len(docs_per_topic) < 2:
                    ...
                    self._save_figure(fig, 'hierarchy', n_topics=len(self.topic_model.get_topic_freq()))
                    return
                ...
                for _, row in docs_per_topic.iterrows():
                    ...
                    if topic != -1:  # Skip outlier topic
                        doc_ids = row['Doc_IDs']
                        if len(doc_ids) > 0:
                            representative_docs.append(self.topic_model._docs[doc_ids[0]])
                ...
                self.topic_model.hierarchical_topics(representative_docs)
        ...
        # Visualize hierarchy
        fig = self._create_hierarchy_visualization(
    ...
    def _create_hierarchy_visualization(self, orientation="left", topics=None, top_n_topics=None, width=1000, height=1000):
        ...
        if not hasattr(self.topic_model, 'hierarchical_topics_'):
        ...
        # Select topic labels
        topic_labels = {topic: self.custom_labels.get(topic, " | ".join([word for word, _ in self.topic_model.get_topic(topic)][:5]))
                       for topic in topics}
        
        # Extract hierarchical topics
        hierarchical_topics = self.topic_model.hierarchical_topics_
        ...
        if len(G.nodes) > 0:
            ...
            try:
                pos = self._hierarchy_pos(G, root_node, orientation=orientation, 
                                         width=1, height=1, 
    ...
    def _hierarchy_pos(self, G, root=None, orientation="left", width=1., height=1., 
    ...
    def save_heatmap(self) -> None:
        ...
        self._save_figure(fig, 'heatmap', n_topics=len(self.topic_model.get_topic_freq()), scale=1)
    ...
    def save_distance_map(self) -> None:
        ...
        try:
            ...
            if hasattr(self.topic_model, 'topic_embeddings_') and isinstance(self.topic_model.topic_embeddings_, np.ndarray):
                ...
                n_topics_available = self.topic_model.topic_embeddings_.shape[0]
                ...
                if self.topic_model.topic_embeddings_.shape[1] > 2:
                    try:
                        ...
                        try:
                            topic_coords = umap_model.fit_transform(self.topic_model.topic_embeddings_)
                            ...
                            try:
                                try:
                                    ...
                                    topic_coords = pca.fit_transform(self.topic_model.topic_embeddings_)
                                    ...
                                    data = self.topic_model.topic_embeddings_
                                    # Center the data
        ...
        self._save_figure(fig, 'intertopic_distance_map', n_topics=len(self.topic_model.get_topic_freq()), scale=1)

    def save_all_visualizations(self, top_n_topics: int = 20):
...
class VisualizationManager:
    ...
    def _setup_output_directory(self):
        # Implementation of _setup_output_directory method
        pass

    def save_barchart(self, top_n_topics: int):
        save_barchart(self.topic_model, top_n_topics, self.custom_labels, len(self.topic_model.topic_model))

    def save_hierarchy(self, orientation="left", topics=None, top_n_topics=None):
        save_hierarchy(self.topic_model, None, "hierarchy", orientation, topics, top_n_topics, False, 1000, 1000, len(self.topic_model.topic_model))

    def save_heatmap(self):
        save_heatmap(self.topic_model, self.custom_labels, len(self.topic_model.topic_model))

    def save_distance_map(self):
        save_distance_map(self.topic_model, self.custom_labels, len(self.topic_model.topic_model))

    def save_all_visualizations(self, top_n_topics: int = 20):
        save_all_visualizations(self.topic_model, self.custom_labels, len(self.topic_model.topic_model), top_n_topics)

    def save_embedding_visualization(self, sample_size=5000, title="Document Embeddings"):
        save_embedding_visualization(self.topic_model.topic_embeddings_, self.topic_model.topics_, self.custom_labels, sample_size, title)

    def save_topic_clusters_plot(self, embeddings=None, sample=5000, hide_document_hover=True, custom_labels=None, width=1600, height=900, title="Topic Clusters"):
        plot_topic_clusters(self.topic_model, embeddings, sample, hide_document_hover, custom_labels, width, height, title)

    def save_topic_clusters(self, embeddings=None, sample=5000, custom_labels=None):
        save_topic_clusters(self.topic_model, embeddings, sample, custom_labels)

# Initialize the visualization manager
viz_manager = VisualizationManager(topic_model, custom_labels)

# Generate individual visualizations
viz_manager.save_barchart(top_n_topics=20)
viz_manager.save_hierarchy(orientation="right")
viz_manager.save_heatmap()
viz_manager.save_distance_map()

# Or generate all visualizations at once
viz_manager.save_all_visualizations(top_n_topics=20) 