...
class TopicVis:
    ...
    def save_hierarchy(self, orientation="right", width=1200, height=800, filename: Optional[str] = None, topics: Optional[List[int]] = None) -> Optional[go.Figure]:
        """
        Visualize and save the hierarchical structure of topics.
        
        Parameters:
            orientation (str): "left", "right", "top", or "bottom".
            width (int): Figure width in pixels.
            height (int): Figure height in pixels.
            filename (str, optional): Name for saving the figure.
            topics (List[int], optional): A list of topic IDs to include. If None, they will be derived from the model.
            
        Returns:
            A Plotly Figure or None if hierarchy could not be created.
        """
        # If topics is None, derive from topic info
        if topics is None:
            try:
                topics_info = self.topic_model.get_topic_info()
                # Exclude outliers (-1) and None values
                topics = sorted([t for t in topics_info["CustomName"].tolist() if t is not None and t != -1])
            except Exception as e:
                self.logger.error(f"Error retrieving topic info: {e}")
                return None

        # Filter topics: ensure get_topic(topic) returns a non-empty list
        valid_topics = []
        for t in topics:
            top = self.topic_model.get_topic(t)
            if top is not None and isinstance(top, list) and len(top) > 0:
                valid_topics.append(t)
            else:
                self.logger.warning(f"Topic {t} returned None or empty list from get_topic; excluding it.")
        topics = valid_topics

        if not topics:
            self.logger.error("No valid topics remain for hierarchy visualization after filtering.")
            return None

        try:
            # Pass the valid topics to the built-in visualize_hierarchy method
            fig = self.topic_model.visualize_hierarchy(orientation=orientation, width=width, height=height, topics=topics)
            self.save_figure(fig, 'hierarchy', scale=2)
            self.logger.info(f"Hierarchical visualization saved as {filename if filename else 'hierarchy'}")
            return fig
        except Exception as e:
            self.logger.error(f"Error visualizing hierarchical topics: {e}")
            self.logger.error(traceback.format_exc())
            fig = go.Figure()
            fig.add_annotation(
                text=f"Error visualizing hierarchical topics: {str(e)}",
                xref="paper", yref="paper", x=0.5, y=0.5,
                showarrow=False, font=dict(size=20)
            )
            fig.update_layout(width=width, height=height)
            self.save_figure(fig, filename if filename else 'hierarchy')
            return fig
    ...
    def save_all_visualizations(self, top_n_topics: int = 20, include_doc_vis: bool = False) -> None:
        ...
        try:
            ...
            self.save_hierarchy(filename="hierarchy",
                                orientation="right", topics=None,
